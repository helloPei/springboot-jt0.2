...
public class RoleServiceImpl implements RoleService {
	...
	public List<CheckBox> findObjects() {
		List<CheckBox> findObjects = roleMapper.findObjects();
		return findObjects;
	}
...
//		Role role = roleMapper.findObjectById(id);
	...
	public int saveObject(Role role, Integer[] menuIds) {
		...
		role.setCreatedUser(ShiroUtils.getPrincipal().getUsername());
...
//		int rows = roleMapper.insertObject(role);
	...
	public int updateObject(Role role,Integer[] menuIds) {
		...
		//2.保存自身角色数据
		role.setModifiedUser(ShiroUtils.getPrincipal().getUsername());
